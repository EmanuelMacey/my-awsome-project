...
export async function createErrand(errandData: {
  ...
  // Invalidate customer errands cache
  await AsyncStorage.removeItem(`errands_${errandData.customerId}`);
  await AsyncStorage.removeItem('admin_errands_cache');
  console.log('🧹 Invalidated customer errands cache');
...
export async function assignRunner(errandId: string, runnerId: string): Promise<void> {
  ...
  if (errandError) {
    console.error('Error fetching errand:', errandError);
    throw errandError;
  }
  ...
  // Invalidate caches
  await AsyncStorage.removeItem(`errands_${errand.customer_id}`);
  await AsyncStorage.removeItem(`driver_errands_${runnerId}`);
  await AsyncStorage.removeItem('admin_errands_cache');
  console.log('🧹 Invalidated errand caches');
...
export async function updateErrandStatus(errandId: string, status: ErrandStatus): Promise<void> {
  ...
  await AsyncStorage.removeItem(`errands_${errand.customer_id}`);
  if (errand.runner_id) {
    await AsyncStorage.removeItem(`driver_errands_${errand.runner_id}`);
  }
  await AsyncStorage.removeItem('admin_errands_cache');
  console.log('🧹 Invalidated errand caches');
