...
import { cancelAbandonedCartReminder } from '../../utils/notificationScheduler';
...
export default function CartScreen() {
  ...
  const handlePlaceOrder = async () => {
    ...
    setLoading(true);
    try {
      console.log('Creating order with cart items:', cart.map(item => ({
        id: item.id,
        name: item.name,
        quantity: item.quantity,
      })));

      // Call createOrder - the API will extract the actual product UUID from cart item IDs
      // FIXED: Pass service_fee parameter
      const order = await createOrder(
        user.id,
        storeId,
        cart.map(item => ({
          item_id: item.id, // This may be "uuid_12oz" format, API will extract the UUID
          quantity: item.quantity,
        })),
        freezePrice(total),
        'cash',
        deliveryAddress,
        undefined,
        freezePrice(subtotal),
        freezePrice(deliveryFee),
        0,
        deliveryLatitude,
        deliveryLongitude,
        userPhone || user.phone || undefined,
        freezePrice(SERVICE_FEE) // FIXED: Pass service fee
      );
      ...
      // Cancel any abandoned cart reminders since order is complete
      console.log('🚫 Cancelling abandoned cart reminders');
      await cancelAbandonedCartReminder();
