...
export default function HomeScreen() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const cachedStores = await AsyncStorage.getItem('stores_cache');
      const cachedCategories = await AsyncStorage.getItem('errand_categories_cache');
      
      if (cachedStores && cachedCategories && !refreshing) {
        try {
          const parsedStores = JSON.parse(cachedStores);
          const parsedCategories = JSON.parse(cachedCategories);
          console.log('⚡ Using cached stores and categories');
          setStores(parsedStores);
          setErrandCategories(parsedCategories);
          setLoading(false);
        } catch (e) {
      ...
      await AsyncStorage.setItem('stores_cache', JSON.stringify(storesData));
      await AsyncStorage.setItem('errand_categories_cache', JSON.stringify(categoriesData));
    } catch (err: any) {
