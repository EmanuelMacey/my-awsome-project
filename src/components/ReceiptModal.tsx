...
export function ReceiptModal({
  ...
  const renderOrderReceipt = () => {
    ...
    // Extract numeric part from order_number (e.g., "ORD-000123" -> 123)
    const orderNumberMatch = order.order_number.match(/\d+/);
    const orderNumberNumeric = orderNumberMatch ? parseInt(orderNumberMatch[0], 10) : order.order_number;
    const orderNumberText = `#${orderNumberNumeric}`;
    ...
    // FIXED: Calculate service fee from order data
    // Service fee = total - (subtotal + delivery_fee + tax - discount)
    const serviceFee = order.service_fee || 200; // Default to 200 if not in database
    ...
    // FIXED: Recalculate total correctly
    const calculatedTotal = order.subtotal + serviceFee + order.delivery_fee + order.tax - (order.discount_amount || 0);
    const totalText = formatCurrency(calculatedTotal);
    ...
    return (
      <View style={styles.receiptContent}>
        ...
        <View style={styles.section}>
          ...
          {/* FIXED: Always show service fee */}
          <View style={styles.listItem}>
            <Text style={styles.listLabel}>Service Fee</Text>
            <Text style={styles.listValue}>{serviceFeeText}</Text>
          </View>
  ...
  const renderErrandReceipt = () => {
    ...
    // Extract numeric part from errand_number
    const errandNumberMatch = errand.errand_number.match(/\d+/);
    const errandNumberNumeric = errandNumberMatch ? parseInt(errandNumberMatch[0], 10) : errand.errand_number;
    const errandNumberText = `#${errandNumberNumeric}`;
    ...
    >
      <View style={styles.modalOverlay}>
        <View style={styles.modalContainer}>
          ...
          {/* Content - FIXED: Removed flex: 1 from ScrollView to ensure content is visible */}
          <ScrollView
